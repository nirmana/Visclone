...
public class Directory {
    ...
    /*public int calcNClonedFile(){

        for(int i=0;i<childDirectoryList.size();i++){
            nclonedFile = nclonedFile + ((Directory)childDirectoryList.get(i)).calcNClonedFile();

        }

        nclonedFile = nclonedFile + clonedFileList.size();
        return nclonedFile;
    }

    public int calcNClone(){

        for(int i=0;i<childDirectoryList.size();i++){
            nclone = nclone + ((Directory)childDirectoryList.get(i)).calcNClone();

        }

        for(int j=0;j<clonedFileList.size();j++){
            nclone = nclone + ((ClonedFile)clonedFileList.get(j)).getCloneList().size();

        }
        System.out.println("Directory "+this.getPath()+"NClone: "+nclone);
        return nclone;
    }*/
